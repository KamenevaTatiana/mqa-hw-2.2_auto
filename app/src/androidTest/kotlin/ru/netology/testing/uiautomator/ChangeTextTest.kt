...
class ChangeTextTest {
    ...
    @Test
    fun testShouldNotChangeTextWithEmptyString() {
        val packageName = MODEL_PACKAGE
        waitForPackage(packageName)
        ...
        device.findObject(By.res(packageName, "buttonChange")).click()
        val resultAfterEmpty = device.findObject(By.res(packageName, "textToBeChanged")).text
        assertEquals(originalText, resultAfterEmpty)

        device.findObject(By.res(packageName, "userInput")).text = "   "
        device.findObject(By.res(packageName, "buttonChange")).click()
        val resultAfterSpaces = device.findObject(By.res(packageName, "textToBeChanged")).text
        assertEquals(originalText, resultAfterSpaces)
    }
